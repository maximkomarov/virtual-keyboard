...
class KeyBoard {
  constructor() {
    ...
    for (let i = 0; i < ASCIIarray.length; i++) {
      ...
      bodyCode += `<div class="button" onclick="keyClick(this)" id = "${button.asciiCode}">
      <span id="textAdd">${button.button.addText}</span><span id="text">${button.button.mainText}</span><span id="type" style="display: none">${button.button.type}</span></div>`;
    }
...
keyClick = function (e) {
  ...
  input.focus();
  if(e.childNodes[3].innerHTML === 'text'){
    input.value += e.childNodes[2].innerHTML  };
};